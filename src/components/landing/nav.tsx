...
export function Nav({
  ...
  return (
    <motion.header
      ...
      className="fixed inset-x-0 top-0 z-50 flex min-w-0 justify-center px-2 pt-3 sm:px-4 sm:pt-4"
    >
      <nav
        className={cn(
          "flex min-w-0 w-full max-w-6xl items-center justify-between rounded-2xl px-2.5 py-2.5 transition-all duration-500 sm:px-4",
          scrolled ? "glass" : "border border-transparent"
        )}
        aria-label="Main navigation"
      >
        <a href={homeHref} className="group flex min-w-0 shrink items-center gap-2 sm:gap-2.5">
          <LogoMark className="h-8 w-8 transition-transform duration-500 group-hover:rotate-[8deg]" />
          <span className="truncate text-[14px] font-semibold tracking-tight text-stone-900 sm:text-[15px]">
            open-proxy
          </span>
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-stone-900/10 bg-white text-stone-700 md:hidden"
          >
            {open ? (
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className="absolute inset-x-0 top-[calc(100%+8px)] max-w-[calc(100vw-1rem)] overflow-hidden rounded-2xl border border-stone-900/10 bg-white p-2 shadow-xl md:hidden"
            >
              {LINKS.map((l) => (
                <a
                  key={l.href}
