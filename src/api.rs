...
pub struct ApiErrorBody {
    ...
    pub message: String,
    /// Always `proxy_error` — the error came from OPENPROXY, not upstream.
    #[serde(rename = "type")]
...
#[openapi(
    info(
        ...
        description = "The operator surface of open-proxy: the settings store, user and key \
                       ...
                       the upstream's, not OPENPROXY's), the HTML page routes, the form-encoded \
                       `/login` and `/logout` browser flow, plain-text `/health`, and the \
                       Prometheus exposition at `/metrics`.",
        license(name = "MIT", url = "https://github.com/shivamsharmahere/Open-Proxy/blob/main/LICENSE"),
        contact(name = "OPENPROXY", url = "https://github.com/shivamsharmahere/Open-Proxy"),
    ),
    paths(
        crate::api_dashboard,
