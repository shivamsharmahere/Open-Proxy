...
`Pool::rebuild(keys)` under the pool write lock, feeding it every stored key —
disabled ones ride along as state carriers (never granted, invisible to
`len`/capacity/stats). Rebuild **carries over per-lane rate state** (`sent` window,
...
plus the lane's endpoint metadata (`Slot { pool, lane, key, base_url,
upstream, endpoint }`) so late cooldown/release after a swap route to the
pool that granted them — no index-out-of-bounds, late ops on a retired pool are
benign. **Invariant**: the superuser always owns ≥1 enabled key on an enabled
[endpoint group](multi-upstream.md), pinning the pool floor (stranding the
last one is a 400), so the pool can never empty. Per-model worker-concurrency limits are a separate concern — see the
