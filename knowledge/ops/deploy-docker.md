...
docker compose up -d     # pulls ghcr.io/shivamsharmahere/Open-Proxy:latest (signed, multi-arch)
docker ps                # STATUS shows (healthy) via the built-in probe
docker logs OPENPROXY-OPENPROXY-1
```
...
— the dev override tags the local build `OPENPROXY:dev` so it can't shadow the
published image.
...
- `HEALTHCHECK` via `OPENPROXY --health` (the binary probes itself; there is
