...
pushes it to `ghcr.io/shivamsharmahere/Open-Proxy`, signs it with keyless cosign,
...
- Bump `version` in `Cargo.toml` and sync `Cargo.lock`
  (`cargo update --package OPENPROXY`). The boot banner and dashboard status
...
git tag -a vX.Y.Z -m "OPENPROXY X.Y.Z" origin/main   # tag the merge commit
...
```sh
docker pull ghcr.io/shivamsharmahere/Open-Proxy:X.Y.Z
docker buildx imagetools inspect ghcr.io/shivamsharmahere/Open-Proxy:X.Y.Z   # amd64 + arm64
docker run -d --name rel-smoke -p 127.0.0.1:8000:8000 ghcr.io/shivamsharmahere/Open-Proxy:X.Y.Z
...
cosign verify ghcr.io/shivamsharmahere/Open-Proxy:X.Y.Z \
  --certificate-identity-regexp 'https://github.com/shivamsharmahere/Open-Proxy/.github/workflows/release.yml@.*' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

# and a downloaded asset against its Sigstore bundle:
cosign verify-blob OPENPROXY-X.Y.Z-linux-amd64.tar.gz \
  --bundle OPENPROXY-X.Y.Z-linux-amd64.tar.gz.sigstore.json \
  --certificate-identity-regexp 'https://github.com/shivamsharmahere/Open-Proxy/.github/workflows/release.yml@.*' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
```

Also check the GitHub Release page: two `OPENPROXY-X.Y.Z-linux-*.tar.gz` assets
plus `OPENPROXY-sbom.spdx.json`, each with its `.sigstore.json` bundle, and
...
git fetch origin main
git tag -fa vX.Y.Z -m "OPENPROXY X.Y.Z" origin/main
git push origin vX.Y.Z
```
