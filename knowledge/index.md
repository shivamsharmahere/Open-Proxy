...
| [prompt-tokens-heuristic-estimate](decisions/prompt-tokens-heuristic-estimate.md) | No upstream usage → prompt tokens estimated as chars÷4 (≥1), marked `source="estimate"`; additive over previously empty series |
...
| [multi-upstream](architecture/multi-upstream.md) | Several OpenAI-compatible APIs per proxy; model-name routing, per-group allowlists, global model toggles, merged catalog |
