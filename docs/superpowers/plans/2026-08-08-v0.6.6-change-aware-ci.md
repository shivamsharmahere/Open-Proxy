...
ci_pr_url=$(gh pr create --repo shivamsharmahere/Open-Proxy \
  --base release/v0.6.6 \
...
test -n "$ci_pr_url"
gh pr edit "$ci_pr_url" --repo shivamsharmahere/Open-Proxy --add-label ci:full
gh pr view "$ci_pr_url" --repo shivamsharmahere/Open-Proxy \
...
reviewed_sha=$(git rev-parse HEAD)
pr_head=$(gh pr view "$ci_pr_url" --repo shivamsharmahere/Open-Proxy --json headRefOid --jq .headRefOid)
...
gh pr checks "$ci_pr_url" --repo shivamsharmahere/Open-Proxy \
...
gh workflow run fuzz.yml --repo shivamsharmahere/Open-Proxy \
...
test "$pr_head" = "$reviewed_sha"
test "$remote_head" = "$reviewed_sha"
gh pr merge "$ci_pr_url" --repo shivamsharmahere/Open-Proxy \
