...
issue_pr_url=$(gh pr create --repo shivamsharmahere/Open-Proxy \
...
test -n "$issue_pr_url"
pr_view=$(gh pr view "$issue_pr_url" --repo shivamsharmahere/Open-Proxy \
...
for route in 'Filter stable = true' 'Filter presentation = false' 'Filter coverage = true' 'Filter docker = true'; do
  gh run view "$ci_run_id" --repo shivamsharmahere/Open-Proxy --log | rg -F -q "$route"
done
...
set -euo pipefail
reviewed_head=$(git rev-parse HEAD)
issue_pr_url=$(gh pr list --repo shivamsharmahere/Open-Proxy --state open --base release/v0.6.6 \
...
~~~
gh issue close 140 --repo shivamsharmahere/Open-Proxy --reason completed \
...
gh issue comment 131 --repo shivamsharmahere/Open-Proxy --body \
