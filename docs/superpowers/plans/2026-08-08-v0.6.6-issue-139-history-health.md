...
- [ ] **Step 1: Preflight ignored evidence and publish the exact ready PR**
  ...
  issue_pr_url=$(gh pr create --repo shivamsharmahere/Open-Proxy --base release/v0.6.6 \
  ...
  pr_view=$(gh pr view "$issue_pr_url" --repo shivamsharmahere/Open-Proxy --json url,state,isDraft,baseRefName,headRefName,headRefOid,files)
...
- [ ] **Step 2: Discover exact-head full CI, CodeQL, and fuzz evidence**
  ...
  set -euo pipefail
  reviewed_head=$(git rev-parse HEAD)
  open_pr=$(gh pr list --repo shivamsharmahere/Open-Proxy --state open --base release/v0.6.6 \
  ...
  for attempt in $(seq 1 60); do
    runs=$(gh api "repos/shivamsharmahere/Open-Proxy/actions/runs?per_page=100")
  ...
  pre_label_ci_created_at=$(gh api "repos/shivamsharmahere/Open-Proxy/actions/runs/$pre_label_ci_run_id" --jq .created_at)
  gh pr edit "$issue_pr_url" --repo shivamsharmahere/Open-Proxy --add-label ci:full
  pr_view=$(gh pr view "$issue_pr_url" --repo shivamsharmahere/Open-Proxy --json headRefOid,labels)
  test "$(jq -r .headRefOid <<<"$pr_view")" = "$reviewed_head"
  ...
  gh workflow run fuzz.yml --repo shivamsharmahere/Open-Proxy --ref work/v0.6.6-139-history-health
  fuzz_run_id=""
  for attempt in $(seq 1 60); do
    runs=$(gh api "repos/shivamsharmahere/Open-Proxy/actions/runs?per_page=100")
  ...
  test -n "$fuzz_run_id"
  gh run watch "$ci_run_id" --repo shivamsharmahere/Open-Proxy --exit-status
  gh run watch "$codeql_run_id" --repo shivamsharmahere/Open-Proxy --exit-status
  gh run watch "$fuzz_run_id" --repo shivamsharmahere/Open-Proxy --exit-status
  ci_view=$(gh run view "$ci_run_id" --repo shivamsharmahere/Open-Proxy --json event,headSha,conclusion,jobs)
  codeql_view=$(gh run view "$codeql_run_id" --repo shivamsharmahere/Open-Proxy --json event,headSha,conclusion,jobs)
  fuzz_view=$(gh run view "$fuzz_run_id" --repo shivamsharmahere/Open-Proxy --json event,headSha,conclusion,jobs)
...
- [ ] **Step 3: Independently review complete evidence and guarded-merge**
  ...
  set -euo pipefail
  reviewed_head=$(git rev-parse HEAD)
  issue_pr_url=$(gh pr list --repo shivamsharmahere/Open-Proxy --state open --base release/v0.6.6 --head work/v0.6.6-139-history-health --json url --jq '.[0].url')
  pr_head=$(gh pr view "$issue_pr_url" --repo shivamsharmahere/Open-Proxy --json headRefOid --jq .headRefOid)
  remote_head=$(git ls-remote origin refs/heads/work/v0.6.6-139-history-health | cut -f1)
  test "$pr_head" = "$reviewed_head" && test "$remote_head" = "$reviewed_head"
  test "$(gh pr view 72 --repo shivamsharmahere/Open-Proxy --json baseRefName --jq .baseRefName)" = main
  main_before=$(git ls-remote origin refs/heads/main | cut -f1)
  gh pr merge "$issue_pr_url" --repo shivamsharmahere/Open-Proxy --merge --match-head-commit "$reviewed_head"
...
- [ ] **Step 4: Close #139 and update #131**
  ...
  set -euo pipefail
  reviewed_head=$(git rev-parse HEAD)
  merged_pr=$(gh pr list --repo shivamsharmahere/Open-Proxy --state merged --base release/v0.6.6 --head work/v0.6.6-139-history-health --json url,headRefOid)
  issue_pr_url=$(jq -r --arg h "$reviewed_head" '[.[] | select(.headRefOid == $h)] | last.url // empty' <<<"$merged_pr")
  test -n "$reviewed_head" && test -n "$issue_pr_url"
  gh issue close 139 --repo shivamsharmahere/Open-Proxy --reason completed --comment "Implemented and merged via $issue_pr_url at $reviewed_head. Derived canonical-presence/poison persistence health, exact 0/1 no-label gauge, catalog-backed Settings guidance, red/green proof, ci:full nine-job CI, CodeQL, exact-ref fuzz, and guarded merge passed."
  gh issue comment 131 --repo shivamsharmahere/Open-Proxy --body "#139 complete: $issue_pr_url merged at $reviewed_head. The history workstream promotion passed derived persistence-health visibility, full CI, CodeQL, exact-ref fuzz, and guarded merge. PR #72 and main remain untouched."
  ~~~
