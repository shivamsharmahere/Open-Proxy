...
ci_log=$(gh run view "$ci_run_id" --repo shivamsharmahere/Open-Proxy --log)
...
set -euo pipefail
reviewed_head=$(git rev-parse HEAD)
issue_pr_url=$(gh pr list --repo shivamsharmahere/Open-Proxy --state open \
  --base release/v0.6.6 --head work/v0.6.6-136-compaction-temp-cleanup \
  --json url --jq '.[0].url')
...
~~~
gh issue close 136 --repo shivamsharmahere/Open-Proxy --reason completed \
...
gh issue comment 131 --repo shivamsharmahere/Open-Proxy \
