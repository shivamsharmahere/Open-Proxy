...
test -n "$issue_pr_url"
gh pr view "$issue_pr_url" --repo shivamsharmahere/Open-Proxy \
...
issue_pr_url=$(gh pr list --repo shivamsharmahere/Open-Proxy --state open \
  ...
  --json url --jq '.[0].url')
test -n "$issue_pr_url"
pr_head=$(gh pr view "$issue_pr_url" --repo shivamsharmahere/Open-Proxy \
...
ci_view=$(gh run view "$ci_run_id" --repo shivamsharmahere/Open-Proxy \
  --json url,event,headSha,conclusion,jobs)
codeql_view=$(gh run view "$codeql_run_id" --repo shivamsharmahere/Open-Proxy \
  --json url,event,headSha,conclusion,jobs)
