...
async fn deadline_header_validation_runs_after_auth_and_before_upstream() {
    ...
    let unauthorized = client()
        .post(proxy.url("/v1/chat/completions"))
        .header("x-openproxy-deadline-ms", "not-a-number")
    ...
    let malformed = client()
        .post(proxy.url("/v1/chat/completions"))
        .bearer_auth("sekrit")
        .header("x-openproxy-deadline-ms", "10.0")
    ...
    let duplicate = client()
        .post(proxy.url("/v1/chat/completions"))
        .bearer_auth("sekrit")
        .header("x-openproxy-deadline-ms", "100")
        .header("x-openproxy-deadline-ms", "200")
...
async fn buffered_deadline_cancels_header_wait_and_releases_inflight_slot() {
    ...
    let expired = client()
        .post(proxy.url("/v1/chat/completions"))
        .header("x-openproxy-deadline-ms", "150")
...
async fn streaming_deadline_stops_an_active_non_idle_stream() {
    ...
    let resp = client()
        .post(proxy.url("/v1/chat/completions"))
        .header("x-openproxy-deadline-ms", "175")
...
async fn streaming_deadline_releases_inflight_when_downstream_is_not_reading() {
    ...
    let unread = client()
        .post(proxy.url("/v1/chat/completions"))
        .header("x-openproxy-deadline-ms", "75")
...
/// `OPENPROXY --health` probes /health on $PORT and exits 0 (healthy) or 1
